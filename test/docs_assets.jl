function test_docs_assets()
    @testset "Docs Assets" begin
        ...
        @test occursin("## Standard Package CI", maintenance)
        @test occursin("## Branch Protection", maintenance)
        @test occursin("## Scope Exceptions", maintenance)
        @test occursin("QUBONotebooks", maintenance)
        @test occursin("Infrastructure-only changes do not require Julia package releases", maintenance)
